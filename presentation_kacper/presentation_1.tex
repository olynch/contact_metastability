...
\begin{frame}
    ...
    \end{theorem}
    \visible<2->{
        The proof of this theorem is quite long. I will be focusing on most interesting details related to proving convergence in probability.
    }
\end{frame}

\section{Sketch of the proof} % Sections can be created in order to organize your presentation into discrete blocks, all sections and subsections are automatically printed in the table of contents as an overview of the talk
...
\begin{frame}
    ...
    \visible<3->{
        Turns out there is a way to do it, which has to do with the fact that time correlations of $\xi(t)$ decay exponentially fast.
    }
\end{frame}
...
\begin{frame}
    ...
    \visible<2->{
        Essentially, rightmost and leftmost infected individuals ``shield'' the entire space between them from outside influence (see gif)
    }
\end{frame}
...
%\begin{frame}
%    \frametitle{Proof}
%    Turns out we can do that by ``anchoring'' $L$ using $\mu_{[-N, \infty)}$ and leveraging its translation invariance. Set
%    \[
